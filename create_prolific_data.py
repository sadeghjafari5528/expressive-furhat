...
ground_truth_df = pd.DataFrame(columns=['story', 'emotion', 'text', 'id'])

c = 0
for story in EMOTIONS:
    for emotion in EMOTIONS + ['none']:
        row = {
            'story': story,
            'emotion': emotion,
            'text': f'data/{story}_{emotion}.mp4',
            'prolific': f'http://38.180.81.115:8000/{c}',
            'id': c
        }
        c += 1
...
attention_row = {
    'story': 'attention',
    'emotion': 'attention',
    'text': 'data/attention_test.mp4',
    'prolific': f'http://38.180.81.115:8000/{c}',
    'id': c
}
c += 1
ground_truth_df = pd.concat([ground_truth_df, pd.DataFrame([attention_row])], ignore_index=True)

for emotion, story in STORY_DICT.items():
    row = {
        'story': emotion,
        'emotion': emotion,
        'text': '\n'.join(story),
